...
export default function AttendanceAllTestPage() {
  ...
  user_id: member.id, // profiles.id 사용
        ...
        registered_at: new Date().toISOString()
            ...
            if (insertError) {
              console.error(`회원 ${member.full_name || member.username} 등록 오류:`, insertError);
              errorCount++;
            } else {
              successCount++;
            }
            ...
            if (cancelError) {
              console.error(`회원 ${member.full_name || member.username} 취소 오류:`, cancelError);
              errorCount++;
            } else {
              successCount++;
            }
            ...
            if (attendError) {
              console.error(`회원 ${member.full_name || member.username} 출석 체크 오류:`, attendError);
              errorCount++;
            } else {
              successCount++;
            }
          ...
          console.error(`회원 ${member.full_name || member.username} 처리 오류:`, error);
          errorCount++;
        }
      }
                      ...
                      return (
                        <div key={uid} className="flex flex-col items-start gap-2 p-2 border rounded bg-gray-50 w-[150px]">
                          <div className="w-full flex items-center justify-between">
                            <div className="text-sm font-medium text-blue-600 truncate">{m.full_name || m.username || uid}</div>
