...
## 🏛️ 6. 홈 메뉴 카드 디자인 표준 (Home Menu Card Standard)

사용자와 관리자 홈(대시보드)의 퀵 메뉴 카드는 시각적 일관성을 유지하고 주목도를 높이기 위해 다음 표준 스타일을 적용합니다.

### 6-1. 기본 스타일 규격
- **테두리선 (Border)**: 기본 테두리는 연한 회색 대신 **조금 진한 파란색 (`border border-blue-200`)**을 기본으로 적용하여 테두리를 뚜렷하게 강조합니다.
- **배경색 (Background)**: 기본 상태에서는 깔끔한 흰색 배경(`bg-white`)을 유지하여 텍스트 가독성을 높입니다.
- **그림자 (Shadow)**: 부드럽고 고급스러운 그림자 효과(`shadow-sm`)를 카드 전체에 적용합니다.
- **모서리 둥글기 (Rounded Corners)**: 사용자 대시보드는 `rounded-[20px]`, 관리자 대시보드는 `rounded-lg` 또는 `rounded-xl` 규격을 유지합니다.

### 6-2. 대시보드 카드 호버 스타일
- 마우스를 올리거나 터치할 때 활성화된 느낌을 주기 위해 **부드러운 호버 배경색 및 테두리 강조**를 설정합니다.
- **공통/사용자**: `hover:border-blue-400 hover:bg-blue-50/50` 등 메뉴 고유 테마별(예: `hover:border-pink-400 hover:bg-pink-50/50` 등) 호버 효과를 다채롭고 조화롭게 융합합니다.
- **관리자**: 각 액션 성격에 맞춰 다채로운 호버 효과(예: `hover:border-green-400 hover:bg-green-50`, `hover:border-orange-400 hover:bg-orange-50` 등)를 매핑합니다.
