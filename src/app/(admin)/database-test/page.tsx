  ...
  useEffect(() => {
    async function testDatabaseConnection() {
      ...
      try {
        ...
        const userData = sessionData.session?.user ? { user: sessionData.session.user } : { user: null };

        results.push({
          test: '현재 사용자 정보',
          success: !sessionError && !!sessionData.session,
          data: userData?.user,
          error: sessionError?.message || (!sessionData.session ? 'Auth session missing!' : null)
        });
