...
- 일반 경기 시스템 관련 문서, UI 문구, 코드 주석에서는 위 용어를 일관되게 사용합니다.
...
- **매일 오전 6시**에 자동으로 실행
- **미리 생성할 일수** 범위 안에 들어오는 해당 요일의 일정을 생성
...
자동 게임 생성 로직의 공정성 강화를 위해 아래 지침을 따릅니다. 이 규칙은 '레벨별', '랜덤', '혼합복식' 세 가지 생성 방식 모두에 동일하게 적용됩니다.

1. 모든 선수 포함
  - 현재 `present` 상태인 모든 선수를 게임 생성 풀에 반드시 포함합니다. 어느 라운드에서도 출석한 선수가 배제되지 않도록 우선순위를 둡니다.

2. 총 게임 수 계산
  - 기본 공식: 총 필요 게임 수 = ceil((총 참가 선수 수 * 1인당 목표 게임 수) / 4)
  - 이 값(`targetMatches`)을 기준으로 기본 매칭을 생성하며, 커버리지(모든 선수가 목표 게임 수 충족)를 위해 별도 보정 단계를 둡니다.

3. 균등 분배 로직
  - 선수별로 현재 생성된 게임 수를 추적합니다.
  - 다음 라운드 구성 시에는 게임 수가 가장 적은 선수들을 우선적으로 배정합니다(오름차순 정렬 후 우선배치).
  - 참가 인원이 4의 배수가 아닐 경우 일부 선수는 한 게임 더 뛰게 될 수 있으나, 추가 게임은 가능한 한 최소화합니다.
