...
export default function ManualPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-tr from-slate-50 via-gray-50 to-blue-50/30 pb-16">
      {/* 프리미엄 헤더 영역 */}
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white py-12 px-6 shadow-md select-none">
        ...
        <div className="relative max-w-6xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="flex items-center gap-2 mb-3 bg-white/20 w-fit px-3 py-1 rounded-full text-xs font-semibold tracking-wide backdrop-blur-sm">
              <Sparkles className="size-3.5" /> SYSTEM GUIDE
            </div>
            <h1 className="text-3xl font-extrabold tracking-tight sm:text-4xl">
              🏸 배드민턴 클럽 시스템 안내서
            </h1>
            <p className="mt-2 text-blue-100 text-base max-w-xl font-light">
              라켓 뚱보단 배드민턴 클럽 시스템을 200% 활용하기 위한 기능 가이드입니다. 출석부터 코인 획득, 실시간 점수판까지 모든 정보를 담았습니다.
            </p>
          </div>
          <Link 
            href="/dashboard" 
            className="flex items-center justify-center gap-2 bg-white/10 hover:bg-white/20 text-white rounded-full px-5 py-2.5 text-sm font-semibold tracking-wide border border-white/20 backdrop-blur-sm transition-all shadow-sm shrink-0 w-fit"
          >
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          {/* 좌측 사이드바 탭 */}
          <div className="lg:col-span-4 bg-white/80 backdrop-blur-md border border-gray-100 rounded-2xl p-4 shadow-sm space-y-2.5 sticky top-20">
              ...
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ChevronRight className={`size-4 opacity-50 transition-transform ${isActive ? 'rotate-90 text-white' : ''}`} />
                  </button>
                );
              })}
            </div>
